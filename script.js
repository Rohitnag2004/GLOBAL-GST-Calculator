...
const taxData = {
    ...
    southkorea: {
        name: "South Korea - Value Added Tax (VAT)",
        currency: "₩",
        description: "South Korea VAT (standard rate currently 10%).",
        rates: [
            { label: "Standard 10%", value: 10, desc: "Standard VAT rate" },
            { label: "Custom Rate", value: "custom", desc: "Enter your rate" }
        ]
    },
    southafrica: {
...
/* -------------------------
   Calculation history (last 5) persisted in localStorage
--------------------------*/
const HISTORY_KEY = 'gst_calc_history_v1';

function pushHistory(entry){
    try {
        let hist = JSON.parse(localStorage.getItem(HISTORY_KEY) || '[]');
        hist.unshift(entry); // newest first
        if(hist.length > 50) hist = hist.slice(0,50); // keep reasonable cap
...
function renderHistory(){
    ...
    if(!list) return;
    const hist = getHistory().slice(0,5); // show last 5
    if(hist.length === 0){
        list.innerHTML = '<div style="color:var(--text-secondary)">No history yet. Perform a calculation to see entries here.</div>';
        return;
    }
    list.innerHTML = hist.map(h => {
        const time = new Date(h.timestamp).toLocaleString();
        return `<div class="history-item" style="padding:8px; border-bottom:1px dashed var(--border-color);">
            <div style="font-weight:700; font-size:0.95rem;">${h.countryName} — ${h.mode === 'exclusive' ? 'Exclusive' : 'Inclusive'} — ${time}</div>
            <div style="font-size:0.9rem; color:var(--text-secondary); margin-top:4px;">
                Base: ${h.currency}${parseFloat(h.base).toFixed(2)} · Tax ${h.taxRate}% → Tax: ${h.currency}${parseFloat(h.tax).toFixed(2)} · Total: ${h.currency}${parseFloat(h.total).toFixed(2)}
            </div>
        </div>`;
    }).join('');
}
...
function exportHistoryCSV(){
    ...
    const rows = [
        ['timestamp','country','mode','base','taxRate','tax','total','currency']
    ];
    hist.forEach(h => rows.push([new Date(h.timestamp).toISOString(), h.country, h.mode, h.base, h.taxRate, h.tax, h.total, h.currency]));
    const csv = rows.map(r => r.map(cell => `"${String(cell).replace(/"/g,'""')}"`).join(',')).join('\n');
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'gst_calculator_history.csv';
...
/* enhance calculate() to push history */
const _origCalculate = calculate;
function calculate_and_history_wrapper(){
    _origCalculate();
    // after calculation, read displayed values and push to history
    try {
        const baseText = document.getElementById('base-amount').textContent.replace(/[^0-9.\-]/g,'');
        const taxText = document.getElementById('tax-amount').textContent.replace(/[^0-9.\-]/g,'');
        const totalText = document.getElementById('total-amount').textContent.replace(/[^0-9.\-]/g,'');
        const taxRateText = document.getElementById('tax-rate-display').textContent.replace('%','').trim();
        const data = {
            timestamp: Date.now(),
            country: currentCountry,
            countryName: taxData[currentCountry] ? taxData[currentCountry].name : currentCountry,
            mode: calculationMode,
            base: parseFloat(baseText) || 0,
            taxRate: parseFloat(taxRateText) || 0,
            tax: parseFloat(taxText) || 0,
            total: parseFloat(totalText) || 0,
            currency: taxData[currentCountry] ? (taxData[currentCountry].currency || '') : ''
        };
        pushHistory(data);
    } catch(e){ console.error('history push failed', e); }
}

/* replace button onclick to use wrapper */
document.addEventListener('DOMContentLoaded', function(){
    const calcBtn = document.querySelector('.calculate-btn');
    if(calcBtn){
        calcBtn.removeEventListener('click', calculate);
        calcBtn.addEventListener('click', calculate_and_history_wrapper);
    }
    // wire clear and export buttons
    const clearBtn = document.getElementById('clear-history-btn');
    if(clearBtn) clearBtn.addEventListener('click', clearHistory);
    const expBtn = document.getElementById('export-history-btn');
    if(expBtn) expBtn.addEventListener('click', exportHistoryCSV);
    renderHistory();
});
